...
const ResizableSplitPane: React.FC<Props> = ({ left, right, isResultReady }) => {
  ...
  return (
    <div ref={containerRef} className="flex flex-row w-full h-full overflow-hidden">
      <div className="shrink-0 h-full flex flex-col min-h-0" style={{ width: `${leftWidth}%` }}>
      ...
      <div 
        className="flex w-6 cursor-col-resize items-stretch justify-center group -ml-3 mr-3 z-10 hover:scale-110 transition-transform flex-shrink-0"
        onMouseDown={startDragging}
      >
        <div className={`w-1 h-24 my-auto rounded-full transition-all duration-300 ${isDragging ? 'bg-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.6)] h-32' : 'bg-slate-800 group-hover:bg-indigo-500/50'}`} />
      </div>
