                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="md:col-span-2 bg-gradient-to-br from-indigo-900/40 to-slate-900/40 border border-indigo-500/30 rounded-xl p-8 flex flex-col shadow-2xl text-left">
                        ...
                        <div className="flex flex-wrap gap-3 mt-auto">
                          {isOwner && (
                            <button onClick={() => setIsShareModalOpen(true)} className="p-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl active:scale-95 flex items-center justify-center transition-all group shadow-lg shadow-indigo-900/40">
                              ...
                              <span className="max-w-0 overflow-hidden group-hover:max-w-xs group-hover:ml-2 transition-all duration-300 font-bold whitespace-nowrap text-left">Share</span>
                            </button>
                          )}
                          {isOwner && (
                            <button onClick={() => setIsChatOpen(true)} className="p-3 bg-white text-slate-950 rounded-xl active:scale-95 transition-all hover:bg-slate-100 flex items-center justify-center group">
                              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m3 21 1.9-5.7a8.5 8.5 0 1 1 3.8 3.8z"/></svg>
                              <span className="max-w-0 overflow-hidden group-hover:max-w-xs group-hover:ml-2 transition-all duration-300 font-bold whitespace-nowrap text-left">{UI_CONTENT.RESULTS.CONSULT_COUNCIL}</span>
                            </button>
                          )}

                          {/* Secondary Layer - Strategic Tools */}
                          <div className="flex items-center gap-2 p-1 bg-slate-950/50 border border-slate-800 rounded-xl">
                            <button onClick={() => setIsElaborationOpen(!isElaborationOpen)} className={`p-2 rounded-lg transition-all ${isElaborationOpen ? 'bg-indigo-500/20 text-indigo-300' : 'text-slate-500 hover:text-slate-300'}`} title="Toggle Elaboration">
                              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>
                            </button>
                            {isOwner && (
                              <button onClick={handleDevelopPlan} disabled={isGeneratingPlan} className="p-2 text-slate-500 hover:text-emerald-400 transition-all disabled:opacity-30" title="Execution Planning">
